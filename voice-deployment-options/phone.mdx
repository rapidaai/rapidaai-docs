...
<Steps>
  ...
  <Step title="General Experience">
    ...
    **Advanced settings** (expand to configure):
    ...
    - **Unclear Speech Wait** — Seconds to wait after interrupted or unclear caller speech before asking the caller to repeat (0.5-5 seconds, default: 0.5s)
    - **Unclear Speech Message** — Message spoken when the caller interrupted but did not continue, or speech was not recognized confidently (default: `I didn't catch that. Could you repeat that?`)
    - **Idle Silence Timeout** — Duration of caller silence before Rapida prompts them (5-60 seconds, default: 30s)
...
<Note>
  Outbound call requests can override saved deployment settings for a single call through `CreatePhoneCallRequest.options`. Use this for per-recipient greetings, language, STT/TTS model, voice, model parameters, or AgentKit connection settings without editing the deployment.
</Note>
