...
| Unclear input timeout | `unclearInputTimeout` | `double` seconds | `0.5` | `0.5`-`5` | Time to wait after interrupted or unclear input before asking the user to repeat. |
| Unclear input message | `unclearInputMessage` | `string` | `I didn't catch that. Could you repeat that?` | Up to 250 characters | Message sent when the assistant could not understand the user input. |
| Idle silence timeout | `idealTimeout` | `uint64` seconds | `30` | `5`-`120` | Time to wait for user input before sending the idle message. |
...
```json
{
  ...
  "options": {
    ...
    "experience.unclear_input_timeout": 0.5,
    "experience.unclear_input_message": "I didn't catch that. Could you repeat that?",
...
| `experience.unclear_input_timeout` | `number` seconds | `unclearInputTimeout` | Time to wait after interrupted or unclear input before asking the caller to repeat. |
| `experience.unclear_input_message` | `string` | `unclearInputMessage` | Message sent when the assistant could not understand the caller input. |
...
<Steps>
  ...
  <Step title="Input is unclear">
    If the user interrupts but does not continue, or speech is not recognized confidently, Rapida waits for `unclearInputTimeout` and then sends `unclearInputMessage`.
  </Step>
...
| Unclear input timeout | `0.5` | Increase only when users often pause after an interruption. |
| Unclear input message | `I didn't catch that. Could you repeat that?` | Keep it short and ask for a repeat. |
