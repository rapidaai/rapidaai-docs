...
This guide explains how to configure webhooks for an assistant.
...
<Steps>
  ...
  <Step title="Save the webhook">
    Review your webhook configuration and click **Configure webhook**.

    Rapida sends the standard webhook payload for each selected event. Custom request body mapping is not required.

    Your webhook is now configured and will be triggered during the selected events.
