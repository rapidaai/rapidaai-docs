...
|----------|------------|------------------|
| `system.current_date` | UTC date (YYYY-MM-DD) | Date-sensitive flows (appointments, policy windows) |
| `system.current_time` | UTC time (HH:MM:SS) | Time-gated behavior |
| `system.current_datetime` | UTC datetime (RFC3339) | Logging-style prompts or strict audit wording |
| `system.day_of_week` | UTC weekday name | Business-hour or weekday-specific logic |
| `system.date_rfc1123` | UTC RFC1123 date string | Interop with strict date formats |
| `system.date_unix` | UTC Unix timestamp (seconds) | Numeric date comparisons |
| `system.date_unix_ms` | UTC Unix timestamp (milliseconds) | High-precision timing context |

### `assistant.*`

| Variable | What it is | Why it is useful |
|----------|------------|------------------|
| `assistant.name` | Assistant name | Consistent self-introduction |
| `assistant.id` | Assistant identifier | Tracing/debug references |
...
| `conversation.identifier` | Conversation identifier | Personalized behavior |
| `conversation.source` | Conversation source | Channel-specific instructions |
| `conversation.direction` | Inbound/outbound direction | Different call-opening behavior |
| `conversation.created_date` | Conversation created datetime | SLA or timing-aware responses |

### `message.*`

| Variable | What it is | Why it is useful |
|----------|------------|------------------|
| `message.language` | Current message language | Multilingual behavior and response control |
| `message.text` | Current message text | Ground immediate response |
