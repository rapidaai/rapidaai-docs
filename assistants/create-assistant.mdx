...
description: "Build a production-ready voice AI assistant — from prompt definition to runtime configuration, tools, knowledge, and deployment."
...
```json
{
  "options": {
    ...
    "model.credential_id": "2230142097179373568",
    "model.connection.transport": "chat_complete",
    "model.connection.endpoint": "/v1/chat/completions"
  }
}
```
...
|------------|------|-------------|
| `model.name` and provider-specific `model.*` keys | varies | Override the saved model provider options for this call, such as model name, temperature, token limits, or response format. |
| `model.credential_id` | `string` / `number` | Use a different saved provider credential for this call. |
| `model.connection.transport` | `string` | Override the model connection transport. |
| `model.connection.endpoint` | `string` | Override the model connection endpoint. |
...
| Unclear input timeout | `0.5 s` | 0.5–5 s | Wait before asking the user to repeat unclear or interrupted input |
| Unclear input message | `I didn't catch that. Could you repeat that?` | Up to 250 characters | Spoken or shown when the assistant could not understand the user |
| Idle message | `Are you there?` | — | Spoken after the idle timeout expires without user input |
| Idle silence timeout | `30 s` | 5–120 s | Time before the idle message triggers |
