...
<Steps>
  ...
  <Step title="Configure security">
    ...
    In Rapida, add the same token as AgentKit metadata. For example, set metadata key `authorization` to the shared token if your server checks the gRPC `authorization` metadata key.
...
| Setting | API field | Default | Allowed values or range | When to change it |
|---|---|---|---|---|
...
```json
{
  "options": {
    ...
    "agentkit.transportSecurity": "TLS",
    "agentkit.tlsVerification": "VERIFY",
    "agentkit.tlsServerName": "agent.your-domain.com",
    "agentkit.connectTimeoutMs": 10000,
    "agentkit.keepaliveTimeMs": 30000,
    "agentkit.keepaliveTimeoutMs": 10000,
    "agentkit.maxRecvMessageBytes": 16777216,
    "agentkit.maxSendMessageBytes": 16777216
  }
}
```

| Option key | Type | Overrides |
|------------|------|-----------|
| `agentkit.metadata` | `object` | gRPC metadata sent to the AgentKit server. Values are merged with saved metadata. |
| `agentkit.transportSecurity` | `string` | Transport security policy, such as `TLS` or `PLAINTEXT`. |
| `agentkit.tlsVerification` | `string` | TLS verification policy, such as `VERIFY` or `SKIP_VERIFY`. |
| `agentkit.tlsServerName` | `string` | TLS server name used for certificate verification. |
| `agentkit.certificate` | `string` | PEM CA certificate. |
| `agentkit.connectTimeoutMs` | `number` | Connect timeout in milliseconds. |
| `agentkit.keepaliveTimeMs` | `number` | Keepalive ping interval in milliseconds. |
| `agentkit.keepaliveTimeoutMs` | `number` | Keepalive timeout in milliseconds. |
| `agentkit.maxRecvMessageBytes` | `number` | Maximum inbound message size. |
| `agentkit.maxSendMessageBytes` | `number` | Maximum outbound message size. |
...
See [Create assistant provider](/api-reference/assistant/create-assistant-provider) for SDK examples.
...
- review [AgentKit scaling](/assistants/agentkit/scaling) before production traffic
...
- [Scale AgentKit backends](/assistants/agentkit/scaling)
