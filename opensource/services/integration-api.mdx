...
```bash
# Uses env/integration.yml by default when run from the repo root
...
```bash
ENV_PATH=docker/integration-api/integration.yml go run cmd/integration/integration.go
```
...
<CardGroup cols={2}>
  ...
  <Card title="Endpoint API" icon="link-2" href="/opensource/services/endpoint-api">
