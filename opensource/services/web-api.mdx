...
```bash
# Uses env/web.yml by default when run from the repo root
go run cmd/web/web.go
```

If you need a different config file, point `ENV_PATH` at it:

```bash
ENV_PATH=docker/web-api/web.yml go run cmd/web/web.go
ENV_PATH=env/web.yml go run cmd/web/web.go
```
