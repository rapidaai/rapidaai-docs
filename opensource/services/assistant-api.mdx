...
The assistant-api reads its configuration from `docker/assistant-api/assistant.yml` (Docker) or `env/assistant.yaml` / `ENV_PATH` when running from source.
...
```bash
# Uses env/assistant.yaml by default when run from the repo root
go run cmd/assistant/assistant.go
```

To use the Docker-local config instead:

```bash
ENV_PATH=docker/assistant-api/assistant.yml go run cmd/assistant/assistant.go
```
