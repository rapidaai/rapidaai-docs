---
title: "Create Web Plugin Deployment"
description: "Create an assistant web plugin deployment from the Rapida SDK."
---
...
<ParamField body="request" type="CreateAssistantDeploymentRequest" required>
  ...
  <Expandable>
    <ParamField body="plugin" type="AssistantWebpluginDeployment" required>
      Web plugin deployment configuration.
      <Expandable>
...
deployment = AssistantWebpluginDeployment(
    ...
    suggestion=["Book a demo", "Talk to support"],
...
response = connection_config.assistant_deployment_client.CreateAssistantWebpluginDeployment(
    CreateAssistantDeploymentRequest(plugin=deployment),
...
func main() {
	...
	response, err := clients.CreateAssistantWebpluginDeployment(
		context.Background(),
		connectionConfig,
		&protos.CreateAssistantDeploymentRequest{
			Deployment: &protos.CreateAssistantDeploymentRequest_Plugin{
				Plugin: &protos.AssistantWebpluginDeployment{
					...
					IdealTimeoutMessage: "Are you still there?",
					MaxSessionDuration:  300,
					Suggestion:          []string{"Book a demo", "Talk to support"},
...
<ParamField body="data" type="AssistantWebpluginDeployment">
  ...
  <Expandable>
    ...
    <ParamField body="suggestion" type="string[]">
      Suggested starter prompts configured for the web widget.
    </ParamField>
    <ParamField body="greeting" type="string">
...
| Code | Meaning |
| --- | --- |
| `1007001` | Invalid request. |
| `1007002` | Unauthenticated request. |
| `1007003` | Missing authentication scope. |
| `1007004` | Invalid `assistantId`. |
| `1007005` | Unable to create assistant web plugin deployment. |
| `1007006` | Invalid `audioProvider`. |
| `1007007` | `idealTimeout` must be between `5` and `120` seconds. |
| `1007008` | `idealTimeoutBackoff` must be between `0` and `5` times. |
| `1007009` | `maxSessionDuration` must be between `180` and `600` seconds. |
