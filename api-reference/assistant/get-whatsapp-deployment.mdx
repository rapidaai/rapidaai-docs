---
title: "Get WhatsApp Deployment"
description: "Get an assistant WhatsApp deployment from the Rapida SDK."
---

Gets the latest WhatsApp deployment for an assistant. Use user-scoped authentication because the operation requires organization and project context.
...
const response = await GetAssistantWhatsappDeployment(connectionCfg, request);
...
response = connection_config.assistant_deployment_client.GetAssistantWhatsappDeployment(
    GetAssistantDeploymentRequest(
...
func main() {
	...
	response, err := clients.GetAssistantWhatsappDeployment(
		context.Background(),
		connectionConfig,
	...
	if err != nil {
		fmt.Printf("error while getting WhatsApp deployment: %+v\n", err)
		return
	}
	fmt.Printf("get WhatsApp deployment response: %+v\n", response)
}
```
...
<ParamField body="data" type="AssistantWhatsappDeployment">
  WhatsApp deployment. If no deployment exists, the deployment field may be omitted.
  <Expandable>
...
| Code | Meaning |
| --- | --- |
| `1013001` | Unauthenticated request. |
| `1013002` | Missing authentication scope. |
| `1013003` | Invalid `assistantId`. |
| `1013004` | Unable to get assistant WhatsApp deployment. |
