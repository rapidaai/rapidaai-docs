---
title: "Get Phone Deployment"
description: "Get an assistant phone deployment from the Rapida SDK."
---
...
response = connection_config.assistant_deployment_client.GetAssistantPhoneDeployment(
    GetAssistantDeploymentRequest(
...
func main() {
	...
	response, err := clients.GetAssistantPhoneDeployment(
		context.Background(),
		connectionConfig,
	...
	if err != nil {
		fmt.Printf("error while getting phone deployment: %+v\n", err)
		return
	}
	fmt.Printf("get phone deployment response: %+v\n", response)
}
```
...
| Code | Meaning |
| --- | --- |
| `1010001` | Unauthenticated request. |
| `1010002` | Missing authentication scope. |
| `1010003` | Invalid `assistantId`. |
| `1010004` | Unable to get assistant phone deployment. |
