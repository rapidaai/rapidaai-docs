---
title: "Create Phone Deployment"
description: "Create an assistant phone deployment from the Rapida SDK."
---

Creates a phone deployment for an assistant. Use user-scoped authentication because the operation requires organization and project context.
...
<ParamField body="request" type="CreateAssistantDeploymentRequest" required>
  Provide the `phone` deployment variant.
  <Expandable>
    <ParamField body="phone" type="AssistantPhoneDeployment" required>
      Phone deployment configuration.
      <Expandable>
        <ParamField body="assistantId" type="uint64" required>
          Assistant ID for the deployment.
        </ParamField>
        <ParamField body="phoneProviderName" type="string" required>
          Telephony provider name, such as `twilio`, `vonage`, `exotel`, `asterisk`, or `sip`.
        </ParamField>
        ...
        <ParamField body="unclearInputTimeout" type="double">
          Seconds to wait after interrupted or unclear input before asking the user to repeat. Must be between `0.5` and `5`.
        </ParamField>
        <ParamField body="unclearInputMessage" type="string">
          Message sent when the assistant could not understand the user input. Maximum length is `250` characters.
        </ParamField>
        <ParamField body="idealTimeout" type="uint64">
          Idle timeout in seconds. Must be between `5` and `120`.
        </ParamField>
...
deployment.setUnclearinputtimeout(0.5);
deployment.setUnclearinputmessage("I didn't catch that. Could you repeat that?");
...
const phoneNumber = new Metadata();
phoneNumber.setKey("phoneNumber");
phoneNumber.setValue("+14155550100");
deployment.setPhoneprovidername("twilio");
deployment.setPhoneoptionsList([phoneNumber]);

const request = new CreateAssistantDeploymentRequest();
request.setPhone(deployment);

const response = await CreateAssistantPhoneDeployment(connectionCfg, request);
...
deployment = AssistantPhoneDeployment(
    ...
    phoneOptions=[Metadata(key="phoneNumber", value="+14155550100")],
...
response = connection_config.assistant_deployment_client.CreateAssistantPhoneDeployment(
    CreateAssistantDeploymentRequest(phone=deployment),
...
func main() {
	...
	response, err := clients.CreateAssistantPhoneDeployment(
		context.Background(),
		connectionConfig,
		&protos.CreateAssistantDeploymentRequest{
			Deployment: &protos.CreateAssistantDeploymentRequest_Phone{
				Phone: &protos.AssistantPhoneDeployment{
					...
					UnclearInputTimeout: proto.Float64(0.5),
					UnclearInputMessage: proto.String("I didn't catch that. Could you repeat that?"),
					IdealTimeout:        30,
					IdealTimeoutBackoff: 2,
					IdealTimeoutMessage: "Are you still there?",
					MaxSessionDuration:  300,
...
| Code | Meaning |
| --- | --- |
| `1005001` | Invalid request. |
| `1005002` | Unauthenticated request. |
| `1005003` | Missing authentication scope. |
| `1005004` | Invalid `assistantId`. |
| `1005005` | Unable to create assistant phone deployment. |
| `1005006` | Invalid `audioProvider`. |
| `1005007` | `idealTimeout` must be between `5` and `120` seconds. |
| `1005008` | `idealTimeoutBackoff` must be between `0` and `5` times. |
| `1005009` | `maxSessionDuration` must be between `180` and `600` seconds. |
| `1005010` | Missing `phoneProviderName`. |
