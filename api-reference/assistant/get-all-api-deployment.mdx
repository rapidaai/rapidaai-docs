---
title: "Get All API Deployments"
description: "Get assistant API deployments from the Rapida SDK."
---
...
func main() {
	...
	if err != nil {
		fmt.Printf("error while getting API deployments: %+v\n", err)
		return
	}
	fmt.Printf("get API deployments response: %+v\n", response)
}
```
...
<ParamField body="id" type="uint64">
  Unique identifier for the API deployment.
</ParamField>
...
| Code | Meaning |
| --- | --- |
| `1016001` | Unauthenticated request. |
| `1016002` | Missing authentication scope. |
| `1016003` | Invalid `assistantId`. |
| `1016004` | Invalid request. |
| `1016005` | Unable to get assistant API deployments. |
